...
// 1) Generalization 2단계:
// => 서브 클래스의 공통 분모(맴버)를 물려주는 용도의 클래스는 직접 인스턴스를 생성하지 못하도록
//    추상 클래스로 선언한다.
// => 개발자가 추상 클래스 여부를 즉각적으로 확인할수 있도록 보통 클래스 이름을
//    "AbstactXxxx" 형식으로 짓는다.
// => 서브 클래스에서 구현할 메서드를 그냥 일반 메서드로 정의하면,
//    서브 클래스를 정의하는 개발자가 해당 메서드를 오버라이딩 한다는 보자을 하지 못한다.
