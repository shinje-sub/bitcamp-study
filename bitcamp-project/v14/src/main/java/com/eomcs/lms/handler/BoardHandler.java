...
public class BoardHandler {
  ...
  // static을 뺀 이유 ? 여러개의 게시판을 따로따로 관리하기 위해
  // 
  Board[] boards;
  int boardCount = 0;
  ...
  // 또한 각 게시판마다 입력 받는 방식을 다르게 할 수 있도록 인스턴스 변수로 선안하자!
  ...
  //  생성자 (constructor)
  // => 인스턴스를 생성할 떄 반드시 호출되는 메서드
  // => new 명령을 실행할 떄 호출될 생성자를 지정할 수 있다.
  // => 주로 의존 객체를 초기화시키는 코드를 넣는다.
  // => 생성자는 리턴 값이 없고, 클래스 이름과 같은 이름으로 메서드를 정의한다.
  // => 생성자를 실행할 떄 사용할 값은 파라미터로 받는다. 
