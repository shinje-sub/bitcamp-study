...
CREATE TABLE t_students (
  ...
  final_edcation VARCHAR(40)  NOT NULL COMMENT '최종학력', -- 최종학력
  major          VARCHAR(40)  NOT NULL COMMENT '전공', -- 전공
  school_name    VARCHAR(40)  NOT NULL COMMENT '최종학교명', -- 최종학교명
  workin         INTEGER      NOT NULL COMMENT '재직여부', -- 재직여부
...
CREATE TABLE t_lessons (
  ...
  total_hour     INTEGER      NOT NULL COMMENT '총강의시간', -- 총강의시간
  day_hour       INTEGER      NOT NULL COMMENT '일강의시간', -- 일강의시간
  quantiy        INTEGER      NOT NULL COMMENT '모집인원', -- 모집인원
...
-- 강의실사진
CREATE TABLE lesson_room_photos (
...
-- 강의실사진
ALTER TABLE lesson_room_photos
  ADD CONSTRAINT PK_lesson_room_photos -- 강의실사진 기본키
    PRIMARY KEY (
      photo_no -- 사진파일번호
    );
...
-- 강의실사진
ALTER TABLE lesson_room_photos
  ADD CONSTRAINT FK_t_lesson_rooms_TO_lesson_room_photos -- 강의실 -> 강의실사진
    FOREIGN KEY (
      lesson_room_no -- 강의실번호
    )
    REFERENCES t_lesson_rooms ( -- 강의실
