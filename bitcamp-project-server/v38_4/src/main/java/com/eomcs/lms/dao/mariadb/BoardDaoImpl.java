...
public class BoardDaoImpl implements BoardDao {
  ...
  public int insert(Board board) throws Exception {
    ...
    try (Statement stmt = con.createStatement()) {
      ...
      // => exccuteUpdate()의 값은 서버에 입력된 데이터의 개수이다.
      int result = stmt.executeUpdate("insert into lms_board(conts) " + ""//
  ...
  public int update(Board board) throws Exception {
    try (Statement stmt = con.createStatement()) {
      ...
      // DBMS에게 데이터 입력하라는 명령을 보낸다.
      // SQL 문법:
      // update 테이블명 set 컬렴명1=값1, 컬럼명2=값2, .... where 조건
      // => exccuteUpdate()의 값은 SQL 명령에 따라 변경된 데이터의 개수이다.
  ...
  public int delete(int no) throws Exception {
    ...
    try (Statement stmt = con.createStatement()) {
      ...
      // DBMS에게 데이터 삭제하라는 명령을 보낸다.
      // SQL 문법:
      // delete from 테이블명 where 조건
      // => exccuteUpdate()의 값은 SQL 명령에 따라 삭제된 데이터의 개수이다.
