...
public class Exam0113 {
  ...
  static class Box<T>{
    ...
    // 그러나 보통 코드를 갈략히하기 위해 한 개의 알파벳으로 표현한다.
  ...
  public static void main(String[] args) {
    ...
    // 클래스에 레퍼런스를 정의할 때 어떤 타입의 값을 다룰 것인지 제안을 걸어야 한다.
    // 즉 지정해야 한다.
    Box<Member> box1;
    ...
    // 메서드를 사용할 때  그 타입 전용 클래스 처럼 쓰면 된다.
    box1.set(new Member("홍길동", 20)); 

    // 따라서 다음과 같이 꺼낼 때도 따로 형변환 할 필요가 없다.
    Member m = box1.get();
    ...
    // 제네릭의 특징이 뭐라고요?
    // => 지정한 타입 전용 클래스로 동작한다.
    // => 그래서 타입이 아닌 경우 메서들 호출할 수 없다.
    //
    //box1.set(new String("Hello!")); // 컴파일 오류!

    // 결론:
    // => 제네릭 문법을 사용하면 Object 타입으로 사용하는 것 보다 편리하다.
    // => 각 타입 별로 따로 클래스를 정의한듯한 효과가 있다.
  }
}
