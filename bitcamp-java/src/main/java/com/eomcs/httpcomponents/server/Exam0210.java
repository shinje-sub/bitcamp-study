// Apache HttpClient 사옹법 : 핸들러에서 동적 콘텐트를 출력하는 웹서버 만들기
...
public class Exam0210 {
  ...
  static class MyRequestHandler implements HttpRequestHandler {
    ...
    @Override
    public void handle(final ClassicHttpRequest request, // 클라이언트 요청 처리 도구
        final ClassicHttpResponse response, // 클라이언트 응답 처리 도구
        final HttpContext context) // HTTP 설정 도구
        throws HttpException, IOException {

      response.setCode(HttpStatus.SC_OK);
      final StringEntity body = new StringEntity("<html><body><h1>안녕하세요!</html></body></h1>",
