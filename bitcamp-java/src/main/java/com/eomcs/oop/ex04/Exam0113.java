...
public class Exam0113 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // String 생성자는 OS에 기본 문자 코드표로 간주하여 변환한다.
    // 따라서 위 예제를 Linux, macOS, Unix 의 
    // CLI(Command Line Interface;) 에서 실행하면
    // 정상적으로 한글 문자열이 출력 되지만,
    // Windows 에서 실행하면 한글 문자열이 깨진다.
    // 이유?
    // Linux, macOS, Unix의 기본 문자 코드표는 UTF-8이다.
    // Windows가 기본적으로 사용하는 문자 코드표는 MS949이다.
    //
    ...
    //    PowerShell 에서는 -Dfile.encoding 옵션을 제데로 처리하지 못한다.
    //    도트(.)를 분리 문자로 인식한다.
  }
}