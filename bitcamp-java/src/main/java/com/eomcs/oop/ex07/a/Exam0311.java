...
class Score4 {
  ...
  // 방법?
  // => 직접 필드에 값을 바꾸게 하지 말고 메서드를 통해 바꾸도록 유도한다.
  // => 이렇게 필드의 값을 바꿀 때 마다 뭔가를 수행해야 하는 경우,
  //    해당 필드에 직접 접근을 막아라!
  // => name, kor, eng, math 필드 같인 경우
  //    값을 설정하고 조회도 해야 하기 때문에 게터/세터가 모두 있다.
  // => sum, aver 필드 같은 경우
  //    값을 조회만 해야 하기 떄문에 게터만 있다.
  // => 이렇게 필드에 대해서  항상 게터/세터를 만드는 것이 아니다.
  //
  // 용어 주의!
...
public class Exam0311 {
  public static void main(String[] args) {
    ...
    // 이제 kor, eng, math 도 비공개 모드이기 떄문에
    // 값을 조회하려면 게터를 사용해야 한다.
    //
    System.out.printf("%s, %d, %d, %d, %d, %.1f\n",
        s1.getName(), s1.getKor(), s1.getMath(), s1.getEng(), s1.getSum(), s1.getAver());
  }
}
