...
public class Servlet02 extends HttpServlet {
  ...
  protected void service(HttpServletRequest request, HttpServletResponse response)
    ...
    // 세션 아이디 보내기
    // => 서버로부터 쿠기를 통해 받은 세션 아이디는
    // 웹 브라우저가 해당 서버에 요청할 때마다
    // 세션 아이디 쿠키를 보낸다.
    // => HTTP 요청 프로토콜
    /*
     * GET /java-web/ex11/s2 HTTP/1.1 Host: localhost:8080 ... Cookie:
     * JSESSIONID=9909D09693CE9E0B8D23BE824313C834 <--- 서버에 세션 아이디 보낸다. Cookie:
     * JSESSIONID=8AC1EFF1AC35FB70E2783018341BD04B
     */

    HttpSession session = request.getSession();
    // 1) 세션 아이디가 있다면,
    // => 세션 아이디가 유효하다면,
    //    => 기존에 생셩한 HttpSession 객체를 리턴한다.
    // => 세션 아이디가 유효하지 않다면,
    //    => 2) 번으로 간다.
    // 2) 세션 아이디가 없다면,
    // => 새 HttpSession 객체를 생성하여 리턴한다.
    // => 응답 프로토콜에 새로 생성한 HttpSession 객체의 세션 아이디를 쿠키로 보낸다.
